...
public abstract class DataUtilities {
    ...
    public static double[][] clone(double[][] source) {
        ...
        int i = 0;
        for (double[] rowSource :source) {
            if (rowSource != null) {
                double[] row = new double[rowSource.length];
                System.arraycopy(rowSource, 0, row, 0, rowSource.length);
                clone[i] = row;
            }
            i++;
        }
    ...
    public static double calculateColumnTotal(Values2D data, int column,
        ...
        for (int validRow : validRows) {
            if (validRow < rowCount) {
                Number n = data.getValue(validRow, column);
                if (n != null) {
                    total += n.doubleValue();
                }
            }
